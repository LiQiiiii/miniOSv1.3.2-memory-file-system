...
PRIVATE void mkfs_fat()
{
	...
	driver_msg.USEMEM = USE_MEM;		  //added by wangshining 2021/12/26
	driver_msg.REQUEST = DIOCTL_GET_GEO;
	driver_msg.BUF = driver_msg.USEMEM ? &meo : &geo; //add by wangshining 2021-12-25
	driver_msg.PROC_NR = proc2pid(p_proc_current);

	hd_ioctl(&driver_msg);

	disp_str("dev size: ");
	if (driver_msg.USEMEM)
	{
		disp_int(meo.size);
	}
	else
	...
	TotalSectors = driver_msg.USEMEM ? meo.size : geo.size; /* partition size in sector */ //modified by wangshining 2021-12-25
