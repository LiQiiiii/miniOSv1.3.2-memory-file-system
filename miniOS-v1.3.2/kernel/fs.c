...
PRIVATE struct inode *mem_root_inode;
...
PUBLIC int real_unlink(const char *pathname); //modified by mingxuan 2019-5-17
...
//~xw
PUBLIC int get_fs_dev(int drive, int fs_type); // modified by wangshining 2021-12-26
...
int get_fs_dev(int drive, int fs_type)
{
	...
	if (drive == MEM_MASTER)
	{
		...
		for (i = 0; i < NR_MEM_PER_DRIVE; i++)
		{
			...
			// disp_int(i);
			// disp_str(" ");
			// disp_int(mem_info.dev_mem_table[i].fs_type);
			// disp_str("\n");
			if (mem_info.dev_mem_table[i].fs_type == fs_type)
			{

				return ((DEV_MEM << MAJOR_SHIFT) | i);
			}
		}
	}
	else
...
PUBLIC void init_fs()
{
	...
	disp_int(sb);
	disp_str(" \n");
	if (sb->magic != MAGIC_V1 && MAJOR(sb->sb_dev) == DEV_HD)
	...
	else if (sb->magic != MAGIC_V1 && MAJOR(sb->sb_dev) == DEV_MEM)
	{
		mkfs(1);
		disp_str("Make file system Done.\n");
		// deleted by mingxuan 2020-10-30
...
PUBLIC void init_fs_mem()
{

	disp_str("Initializing mem file  system...  \n");
	...
	//for (; sb < &super_block[NR_SUPER_BLOCK]; sb++)				//deleted by mingxuan 2020-10-30
	//	sb->sb_dev = NO_DEV;										//deleted by mingxuan 2020-10-30
	int orange_dev = get_fs_dev(MEM_MASTER, ORANGE_TYPE); //added by mingxuan 2020-10-27
	/* load super block of ROOT */
	//read_super_block(ROOT_DEV);		// deleted by mingxuan 2020-10-27
	read_super_block(orange_dev); // modified by mingxuan 2020-10-27
	//sb = get_super_block(ROOT_DEV);	// deleted by mingxuan 2020-10-27
	...
	//root_inode = get_inode(ROOT_DEV, ROOT_INODE);	// deleted by mingxuan 2020-10-27
	mem_root_inode = get_inode(orange_dev, ROOT_INODE); // modified by mingxuan 2020-10-27
}
/*****************************************************************************
 *                                mkfs
 *****************************************************************************/
...
PRIVATE void mkfs(int use_mem)
{
	...
	int orange_dev = use_mem ? get_fs_dev(MEM_MASTER, ORANGE_TYPE)
							 : get_fs_dev(PRIMARY_MASTER, ORANGE_TYPE); //added by mingxuan 2020-10-27

	/* get the geometry of ROOTDEV */
	struct part_info geo;
	struct t_dev_mem meo; //add by wangshining 2021-12-25

	driver_msg.type = DEV_IOCTL;
	driver_msg.USEMEM = use_mem; //add by wangshining 2021-12-25
	...
	driver_msg.BUF = use_mem ? &meo : &geo; //add by wangshining 2021-12-25
	...
	disp_str(" dev size: ");
	//added by wangshining 2021-12-29
	if (use_mem)
	{
		disp_int(meo.size);
	}
	...
	sb.nr_sects = use_mem ? meo.size : geo.size; /* partition size in sector */ //modified by wangshining 2021-12-25
	sb.nr_imap_sects = 1;
	...
	// disp_str(" nr_smap_sects:");
	// disp_int(sb.nr_smap_sects);
	// disp_str(" nr_inode_sects:");
	// disp_int(sb.nr_inode_sects);
	...
	// disp_str(" inode_isize_off:");
	// disp_int(sb.inode_isize_off);
	// disp_str(" inode_start_off:");
	// disp_int(sb.inode_start_off);
	// disp_str(" dir_ent_size:");
	// disp_int(sb.dir_ent_size);
	// disp_str("\n");
	struct dir_entry de;
	...
	// disp_str("devbase:");
	// disp_int(meo.start_mem * 2);
	// disp_str("00");

	// disp_str(" sb:");
	// disp_int((meo.start_mem + 1) * 2);
	// disp_str("00");
	// disp_str(" imap:");

	// disp_int((meo.start_mem + 1 + 1) * 2);
	// disp_str("00");
	// disp_str(" smap:");

	// disp_int((meo.start_mem + 1 + 1 + sb.nr_imap_sects) * 2);
	// disp_str("00\n");

	// disp_str("        inodes:");
	// disp_int((meo.start_mem + 1 + 1 + sb.nr_imap_sects + sb.nr_smap_sects) * 2);
	// disp_str("00");

	// disp_str(" 1st_sector:");
	// disp_int(meo.start_mem + sb.n_1st_sect);
	// disp_str(" ");
	// disp_int((meo.start_mem + sb.n_1st_sect) * 2);
	// disp_str("00\n");
	if (use_mem)
	{
		//*****************inode-map*****************
		...
		for (i = 0; i < 2; i++) //modified by mingxuan 2019-5-22
			fsbuf[0] |= 1 << i;
...
PRIVATE int rw_sector(int io_type, int dev, u64 pos, int bytes, int proc_nr, void *buf)
{
	...
	driver_msg.BUF = buf;
	driver_msg.USEMEM = (ma_dev == DEV_MEM) ? USE_MEM : NOT_USE_MEM;
	// assert(dd_map[MAJOR(dev)].driver_nr != INVALID_DRIVER);
	// send_recv(BOTH, dd_map[MAJOR(dev)].driver_nr, &driver_msg);
	// if (ma_dev == DEV_MEM)
	// {
	// 	disp_str("\n");
	// 	disp_int(driver_msg.USEMEM);
	// 	disp_str("\n");
	// }
...
PRIVATE int strip_path(char *filename, const char *pathname, struct inode **ppinode)
{
	...
	char fs_name[12]; //modified by mingxuan 2020-10-18
	int len = (pathlen < 12) ? pathlen : 12;
	...
	for (i = 0; i < len; i++)
	{
		...
		{
			//dev_name[i] = path[i];
			fs_name[i] = pathname[i]; //modified by mingxuan 2020-10-18
		}
	}
	fs_name[i] = '\0';
	// disp_str(fs_name);
	...
	else
	{
		for (i = 0; i < NR_FS; i++)
		{
			if (!strcmp(fs_name, vfs_table[i].fs_name))
			{
				index = i;
				s = s + a;
			}
	...
	*t = 0;
	...
	// disp_int(index);
	// disp_str("  ");
	// disp_int(vfs_table[index].sb->sb_dev);
	// disp_str("\n");
	if (MAJOR(vfs_table[index].sb->sb_dev) == DEV_MEM)
	{
		*ppinode = mem_root_inode;
	}
	else
	{
		*ppinode = root_inode;
...
PUBLIC void read_super_block(int dev) //modified by mingxuan 2020-10-30
{
	...
	driver_msg.USEMEM = (ma_dev == DEV_MEM) ? USE_MEM : NOT_USE_MEM; //added by wangshining 2021-12-28
	...
	*/
	...
	for (i = 0; i < NR_SUPER_BLOCK; i++)
		if (((super_block[i].sb_dev == ma_dev) || (MAJOR(super_block[i].sb_dev) == ma_dev)) && super_block[i].fs_type == ORANGE_TYPE)
			break;
	if (i == NR_SUPER_BLOCK)
...
PRIVATE struct inode *get_inode(int dev, int num)
{
	...
	// disp_str(" isize:");
	// disp_int(q->i_size);
	// disp_str(" i_start_sect:");
	// disp_int(q->i_start_sect);
	// disp_str(" i_nr_sects:");
	// disp_int(q->i_nr_sects);
	// disp_str("\n");
	// for (int i = 1; i <= 4; i++)
	// {
	// 	struct inode *pi =
	// 		(struct inode *)((u8 *)fsbuf +
	// 						 ((i) % (SECTOR_SIZE / INODE_SIZE)) * INODE_SIZE);
	//added by wangshining 2021-12-29
	// disp_str("node:");
	// disp_int(i);
	// disp_str(" imode:");
	...
	// disp_int(pi->i_size);
	// disp_str(" i_start_sect:");
	// disp_int(pi->i_start_sect);
	// disp_str(" i_nr_sects:");
	// disp_int(pi->i_nr_sects);
	// disp_str(" \n");
...
PRIVATE int alloc_imap_bit(int dev)
{
	...
	char fsbuf[SECTOR_SIZE]; //local array, to substitute global fsbuf. added by xw, 18/12/27
	int *p = mem_fs[0].lin_addr;

	// for (int s = 0; s < 10; s++, p += (SECTOR_SIZE >> 2))
	// {
	// 	disp_int(s);
	// 	disp_str("fsbuf:");
	// 	disp_int(p);
	...
	// 	fsbuf[0] = 128;
	// 	*p = 128;
	// 	// memcpy(p, fsbuf, (SECTOR_SIZE >> 2));
	// 	phys_copy(fsbuf, p, (SECTOR_SIZE >> 2));
	// 	//RD_SECT(dev, s, fsbuf);
