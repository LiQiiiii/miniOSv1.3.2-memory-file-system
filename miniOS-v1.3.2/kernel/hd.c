...
PUBLIC void hd_rdwt(MESSAGE *p)
{
	...
	// disp_int(p->USEMEM);
	// disp_str("\n");
	//add by wangshining 2021-12-25
	//这里还是以扇区为单位进行读写，开始读的地址为t_mem_info.base + sect_mt * SECTOR_SIZE
	u32 sect_mt = (u32)(pos >> SECTOR_SIZE_SHIFT);
	void *la = (void *)va2la(p->PROC_NR, p->BUF);
	struct hd_cmd cmd;
	if (p->USEMEM)
	{
		sect_mt += mem_info.dev_mem_table[p->DEVICE].start_mem;
	}
	/*
		这里由于传进来的position是 扇区地址*SECTOR_SIZE
		并且文件系统的super block、i_node map也是通过这个函数进行读写操作的，所以在这个函数内进行分支，调用接口从内存中读写数据
	*/
	...
	if (p->USEMEM)
	{
		if (p->type == DEV_READ)
		{
			read_sector_mem(membuf, sect_mt, p->CNT, p->PROC_NR);
			// disp_str("\n       readbuf: ");
			// disp_str(membuf);
			// disp_str("   ");
			phys_copy(la, membuf, p->CNT);
		}
		else
		{
			phys_copy(membuf, la, p->CNT);
			// disp_str("\n        writebuf: ");
			// disp_str(membuf);
			// disp_str("  ");
			write_sector_mem(membuf, sect_mt, p->CNT, p->PROC_NR);
		}
	}
	else
	{
	...
	// u32 sect_i = sect_mt; //delete by wangshining 2021-12-28
	int bytes_left = p->CNT;
	// int *mem_base = mem_info.base; //delete by wangshining 2021-12-28

	while (bytes_left && !p->USEMEM)
	{
		...
		if (p->type == DEV_READ)
		{
			if (p->USEMEM)
			{
				// u32 off = (sect_i * SECTOR_SIZE) >> 2; //delete by wangshining 2021-12-28
				// disp_int(mem_base + off); //delete by wangshining 2021-12-28
				// phys_copy(membuf, mem_base + off, SECTOR_SIZE); //delete by wangshining 2021-12-28
				//add by wangshining 2021-12-15
				// phys_copy(la, membuf, bytes);
			}
			else
			{
				interrupt_wait();
		...
		else
		{
			...
			if (p->USEMEM)
			{	//add by wangshining 2021-12-25
				// phys_copy(membuf, la, bytes);
				// //add by wangshining 2021-12-25
				// phys_copy(mem_base + sect_i * SECTOR_SIZE, membuf, SECTOR_SIZE);
			}
			else
			{
		...
		//deleted by wangshining 2021-12-28
		// if (p->USEMEM)
		// {
		// 	sect_i++;
		// }
...
PRIVATE void hd_rdwt_real(RWInfo *p)
{
	...
	sect_nr += p->msg->DEVICE < MAX_PRIM ? hd_info[drive].primary[p->msg->DEVICE].base : hd_info[drive].logical[logidx].base;
...
PRIVATE int out_hd_queue(HDQueue *hdq, RWInfo **p)
{
	...
	*p = hdq->front;
	if (hdq->front == hdq->rear)
	{ //put out the last node
		hdq->front = hdq->rear = NULL;
	}
	else
	{
...
PUBLIC void hd_ioctl(MESSAGE *p)
{
	...
	struct t_mem_info *memi = &mem_info;

	if (p->REQUEST == DIOCTL_GET_GEO)
	{

		//added by wangshining 2021-12-26
		//这里跟hd_rdwt的改动方法一样，在这里加个分支
		if (p->USEMEM)
		{
			void *dst = va2la(p->PROC_NR, p->BUF);
			void *src = va2la(proc2pid(p_proc_current), &memi->dev_mem_table[device]);
			phys_copy(dst, src, sizeof(struct t_dev_mem));
		}
		else
		{
			void *dst = va2la(p->PROC_NR, p->BUF);
